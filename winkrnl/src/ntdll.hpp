...
#pragma warning(disable : 4005) // level 4 error - nonstandard extension 'single line comment' was used
#if defined(__ICL)
...
typedef enum _EVENT_TRACE_INFORMATION_CLASS {
    ...
    *PEVENT_TRACE_INFORMATION_CLASS;
...
typedef BOOLEAN (*PRTL_CONFLICT_RANGE_CALLBACK)(
    IN PVOID Context,
    IN PRTL_RANGE Range);
...
#define GDI_CLIENT_TYPE_FROM_HANDLE(Handle) ((ULONG)(Handle) & ((GDI_HANDLE_ALTTYPE_MASK << GDI_HANDLE_ALTTYPE_SHIFT) | (GDI_HANDLE_TYPE_MASK << GDI_HANDLE_TYPE_SHIFT)))
...
typedef ULONG (*PCSR_CALLBACK_ROUTINE)(
...
#define RtlpOwnerAddrSecurityDescriptor(SD) \
    (((SD)->Control & SE_SELF_RELATIVE) ? ((((SECURITY_DESCRIPTOR_RELATIVE*)(SD))->Owner == 0) ? ((PSID)NULL) : (PSID)RtlOffsetToPointer((SD), ((SECURITY_DESCRIPTOR_RELATIVE*)(SD))->Owner)) : (PSID)((SD)->Owner))

#define RtlpGroupAddrSecurityDescriptor(SD) \
    (((SD)->Control & SE_SELF_RELATIVE) ? ((((SECURITY_DESCRIPTOR_RELATIVE*)(SD))->Group == 0) ? ((PSID)NULL) : (PSID)RtlOffsetToPointer((SD), ((SECURITY_DESCRIPTOR_RELATIVE*)(SD))->Group)) : (PSID)((SD)->Group))

#define RtlpSaclAddrSecurityDescriptor(SD) \
    ((!((SD)->Control & SE_SACL_PRESENT)) ? (PACL)NULL : (((SD)->Control & SE_SELF_RELATIVE) ? ((((SECURITY_DESCRIPTOR_RELATIVE*)(SD))->Sacl == 0) ? ((PACL)NULL) : (PACL)RtlOffsetToPointer((SD), ((SECURITY_DESCRIPTOR_RELATIVE*)(SD))->Sacl)) : (PACL)((SD)->Sacl)))

#define RtlpDaclAddrSecurityDescriptor(SD) \
    ((!((SD)->Control & SE_DACL_PRESENT)) ? (PACL)NULL : (((SD)->Control & SE_SELF_RELATIVE) ? ((((SECURITY_DESCRIPTOR_RELATIVE*)(SD))->Dacl == 0) ? ((PACL)NULL) : (PACL)RtlOffsetToPointer((SD), ((SECURITY_DESCRIPTOR_RELATIVE*)(SD))->Dacl)) : (PACL)((SD)->Dacl)))
...
#define RtlpPropagateControlBits(NewSD, OldSD, Bits) \
    (NewSD)->Control |= ((OldSD)->Control & (Bits))
